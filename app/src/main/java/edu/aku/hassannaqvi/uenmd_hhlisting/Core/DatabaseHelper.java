...
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.BLRandomContract.singleRandomHH;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.EnumBlockContract;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.EnumBlockContract.EnumBlockTable;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.ListingContract;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.ListingContract.ListingEntry;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.SignupContract;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.SignupContract.SignUpTable;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.TeamsContract;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.TeamsContract.SingleTaluka;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.UsersContract;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.UsersContract.UsersTable;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.VersionAppContract;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.VersionAppContract.VersionAppTable;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.VerticesContract;
import edu.aku.hassannaqvi.uenmd_hhlisting.Contracts.VerticesContract.SingleVertices;
...
// Create a new map of values, where column names are the keys
        ...
        values.put(ListingEntry.COLUMN_NAME_HHDATETIME, lc.getSysdate());
        ...
        values.put(ListingEntry.COLUMN_SYNCED, "");
...
// Create a new map of values, where column names are the keys
        ...
        values.put(singleRandomHH.COLUMN_RANDOMDT, lc.getSysdate());
...
// Which row to update, based on the title
    ...
    public JSONArray getUnsyncedListings() throws JSONException {
        SQLiteDatabase db = this.getReadableDatabase(DATABASE_PASSWORD);
        Cursor c = null;
        String[] columns = null;
        ...
        String whereClause;
        // whereClause = null;
        whereClause = ListingEntry.COLUMN_SYNCED + " = '' ";
        ...
        String orderBy = ListingEntry._ID + " ASC";
        ...
        while (c.moveToNext()) {
            ...
            ListingContract listings = new ListingContract();
            allListings.put(listings.hydrate(c, 0).toJSONObject());
    ...
    public int syncEnumBlocks(JSONArray Enumlist) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                long rowID = db.insert(EnumBlockTable.TABLE_NAME, null, values);
                if (rowID != -1) insertCount++;
            }
        } catch (Exception e) {
        } finally {
